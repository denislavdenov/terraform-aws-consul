...
resource "aws_instance" "consul_servers_dc1" {
  ami                         = "${var.ami["server"]}"
  ...
  private_ip                  = "${var.IP_DC1["server"]}${count.index + 1}"
...
resource "aws_instance" "nginx_clients_dc1" {
  ami                         = "${var.ami["client"]}"
  instance_type               = "${var.instance_type}"
  subnet_id                   = "${var.subnet_id}"
  key_name                    = "${aws_key_pair.key.id}"
  vpc_security_group_ids      = "${var.security_group_id}"
  iam_instance_profile        = "${aws_iam_instance_profile.consul.id}"
  private_ip                  = "${var.IP_DC1["client"]}${count.index + 1}"
  associate_public_ip_address = true
  count                       = "${var.nginx_client_dc1_count}"
